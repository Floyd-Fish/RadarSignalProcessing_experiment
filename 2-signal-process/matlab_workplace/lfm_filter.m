...
lfm_n_f = filter(d.Coefficients, 1, [lfm_n zeros(1, 274)]);
lfm_n_f = lfm_n_f(274+1:end);
...
figure;
subplot(4, 1, 1); plot(t.*1000, lfm_n); title('time domain');
xlabel('time / ms'); ylabel('Amplitude / V');
subplot(4, 1, 2); plot(fv./1000, s_n(1:N/2)); title('spectrum');
xlabel('frequency / kHz');  ylabel('Amplitude / dBV'); % set(gca, 'xscale', 'log');
subplot(4, 1, 3); plot(t.*1000, lfm_n_f); title('time domain');
xlabel('time / ms'); ylabel('Amplitude / V');
subplot(4, 1, 4); plot(fv./1000, s_n_f(1:N/2)); title('spectrum');
xlabel('frequency / kHz');  ylabel('Amplitude / dBV'); % set(gca, 'xscale', 'log');
